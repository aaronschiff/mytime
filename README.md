...
3. `MenuBarExtra` always renders its label's SF Symbol images as forced "template" (monochrome, auto-tinted) images — neither SwiftUI's `.foregroundStyle()` nor `.renderingMode(.original)` can override this (confirmed via [Apple Developer Forums thread 738716](https://developer.apple.com/forums/thread/738716)). A genuinely colored icon (the red stop-square) requires bypassing `Image(systemName:)` and building an `NSImage` directly with an explicit `NSImage.SymbolConfiguration(paletteColors:)` and `isTemplate = false` — see `statusIcon` in `MyTimeMenuBarApp.swift`.
